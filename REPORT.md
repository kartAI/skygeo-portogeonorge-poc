...
4. **`portolan add` beregner GeoParquet-avledet STAC-metadata (feature_count, geometry_type) uten a registrere selve GeoParquet-filen som asset.** `check --fix` konverterer sa kildefilen til GeoParquet, og en andre `portolan add`-kjoring kreves for a registrere den resulterende parquet-en og thumbnail-en. Pipelinen kjorer derfor `add -> check --fix -> add` per collection i stedet for planens ett-linjers `add`+`check --fix`.

   **Oppfolging (oppdaget etter forste fulle kjoring, na fikset):** nar `<navn>.gpkg` og den avledede `<navn>.parquet` har samme filnavnstamme, "vinner" `add` sin asset-nokkel-kollisjon inkonsekvent -- noen ganger blir parquet riktig registrert som collection-ens data-asset, andre ganger forblir gpkg registrert og parquet forblir usporet helt til noen manuelt kjorer `add` pa nytt, uavhengig av `--force`/`--merge-strategy`. Utfallet korrelerer ikke med datastorrelse, geometri-type eller hvilken pipeline-kjoring som prosesserte datasettet; det ser ut til a avhenge av filsystem-enumereringsrekkefolge internt i portolan-cli 0.8.0, ikke noe som kan styres via CLI-flagg. 7 av 28 collections i denne katalogen (inkludert `korallrev` og `sefrak-bygninger`) var rammet og er rettet retroaktivt (fjernet den na-redundante `.gpkg`-en og kjort `add --force` pa nytt -- se `portolan_runner.py`s `_promote_parquet_if_orphaned`). Pipelinen kjorer na denne korreksjonen automatisk per collection og en gang til, katalog-bredt, etter den avsluttende `check --fix` (se `pipeline.py`), sa fremtidige kjoringer ikke rammes pa samme måte.
